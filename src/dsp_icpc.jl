...
- `n_sat_high_cons`: number of consecutive samples the waveform is saturated at high of FADC range
"""
function dsp_ged_auto(data::Q, config::DSPConfig, τ::Quantity{T}) where {Q<:Table,T<:Real}
    # get config parameters
    bl_window = config.bl_window
    ...
    qdrift_int_length = config.qdrift_int_length 
    lq_int_length = config.lq_int_length 
    rise_time_auto = config.rise_time_auto
    ft_time_auto = config.flat_top_time_auto
    sg_length_auto = config.sg_length_auto 
    ...
    # saturation parameter determination
    bit_depth = config.kwargs_pars.fc_bit_depth # of FlashCam FADC
    sat_low, sat_high = 0, (2^bit_depth - bit_depth) * first(presum_rate_value)
    sat_stats = saturation.(wvfs_pre, sat_low, sat_high)
    ...
    # baseline restoration, wvfs = wvfs - bl
    wvfs_pre = shift_waveform.(wvfs_pre, -bl_stats.mean)
    wvfs_wdw = shift_waveform.(wvfs_wdw, -bl_stats.mean ./ presum_rate_value)

    # waveform extrema determination
    wvf_pre_stats = extremestats.(wvfs_pre)
    wvf_wdw_stats = extremestats.(wvfs_wdw)

    # decay time determination
    decay_stats = tailstats.(wvfs_pre, leftendpoint(tail_window), rightendpoint(tail_window))

    # pole-zero correction, wvfs = pzc(wvfs)
    deconv_flt = InvCRFilter(τ)
    wvfs_pre = deconv_flt.(wvfs_pre)
    wvfs_wdw = deconv_flt.(wvfs_wdw)
    ...
    t50 = get_threshold(wvfs_wdw, wvf_wdw_stats.max .* 0.5; mintot=config.kwargs_pars.tx_mintot)
    t80 = get_threshold(wvfs_wdw, wvf_wdw_stats.max .* 0.8; mintot=config.kwargs_pars.tx_mintot)
    t90 = get_threshold(wvfs_wdw, wvf_wdw_stats.max .* 0.9; mintot=config.kwargs_pars.tx_mintot)
    t50_pre = get_threshold(wvfs_pre, wvf_pre_stats.max .* 0.5; mintot=config.kwargs_pars.tx_mintot)

    # drift time determination
    # drift_time = uconvert.(u"ns", t90 - t0)

    # Q-drift determination
    qdrift = get_qdrift(wvfs_wdw, t0, qdrift_int_length; pol_power=config.kwargs_pars.int_interpolation_order, sign_est_length=config.kwargs_pars.int_interpolation_length)
    ...
    uflt_trap = TrapezoidalChargeFilter(rise_time_auto, ft_time_auto)
    wvfs_trap = uflt_trap.(wvfs_pre)

    trap_stats = extremestats.(wvfs_trap)

    signal_estimator = SignalEstimator(PolynomialDNI(config.kwargs_pars.sig_interpolation_order, config.kwargs_pars.sig_interpolation_length))
    e_trap = signal_estimator.(wvfs_trap, t50_pre .+ (rise_time_auto + ft_time_auto/2))

    # current reconstruction, second order polynominal and first derivative
    uflt_sg = SavitzkyGolayFilter(sg_length_auto, sg_flt_degree, 1)
    wvfs_sg = uflt_sg.(wvfs_wdw)

    a_sg = get_wvf_maximum.(wvfs_sg, leftendpoint(current_window), rightendpoint(current_window))

    # triggering
    uflt_sg = SavitzkyGolayFilter(4 * sg_length_auto, sg_flt_degree, 1)
    wvfs_sg = uflt_sg.(wvfs_pre)

    inTrace_pileUp = get_intracePileUp(wvfs_sg, inTraceCut_std_threshold, bl_window; mintot=config.kwargs_pars.intrace_mintot)

    # current time point determination
    #thres = maximum.(wvfs_sg.signal) .* 0.5
    #t50_current = get_threshold(wvfs_sgflt_deriv, thres; mintot=config.kwargs_pars.tx_mintot)

    # wavefrom inversion, wvfs = -wvfs
    wvfs_pre = multiply_waveform.(wvfs_pre, -1.0)
    wvfs_wdw = multiply_waveform.(wvfs_wdw, -1.0)

    # energy reconstruction
    wvfs_trap = uflt_trap.(wvfs_pre)
    e_max_inv = maximum.(wvfs_trap.signal)
    ...
    return TypedTables.Table(
        ...
        qdrift=qdrift, lq=lq,
        # energy
        e_trap_max=trap_stats.max, trap_tmax=trap_stats.tmax, e_trap=e_trap,
        # amplitude
        a_sg=a_sg,
        # trigger positions
        inTrace_intersect=inTrace_pileUp.intersect, inTrace_n=inTrace_pileUp.n,
        # inverse parameters
        e_max_inv=e_max_inv, t0_inv=t0_inv
    )
end
export dsp_ged_auto